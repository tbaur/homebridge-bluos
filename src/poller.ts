...
export class DevicePoller {
  private readonly options: DevicePollerOptions

  private currentEndpoint: Endpoint

  private observation: PlayerObservation | undefined
  ...
  setEndpoint(endpoint: Endpoint): void {
    ...
    this.options.log.info(
      `${forLog(this.options.displayName)} moved to ${endpoint.host}:${endpoint.port}`,
    )
  ...
  start(): void {
    ...
    this.loop = this.run().catch((error: unknown) => {
      this.options.log.error(
        `${this.label()} polling stopped after an unexpected error, so its state will `
        + `no longer update until Homebridge restarts: ${describeError(error)}`,
      )
    })
  }
      ...
      try {
        ...
        if (this.generation !== generation) {
          ...
          this.options.log.debug(
            `${forLog(this.options.displayName)} discarding a poll response that crossed a write`,
          )
          this.etag = undefined
          continue
        }
  ...
  private async handleFailure(error: unknown): Promise<void> {
    ...
    if (this.consecutiveFailures === 1) {
      this.options.log.debug(`${this.label()} poll failed: ${describeError(error)}`)
    }
    ...
    // new address.
    if (this.consecutiveFailures >= POLL_FAILURES_BEFORE_UNKNOWN) {
      this.options.onUnreachable(error)
    }
    if (this.consecutiveFailures >= FAILURES_BEFORE_REDISCOVERY) {
    ...
    const ceiling = Math.min(POLL_BACKOFF_MAX_MS, POLL_BACKOFF_BASE_MS * 2 ** exponent)
   ...
   */
  private async tryRediscovery(): Promise<boolean> {
    const now = Date.now()
    if (this.stopped || now - this.lastRediscoveryAt < REDISCOVERY_MIN_INTERVAL_MS) {
      return false
    }
    ...
    try {
      ...
      this.options.log.debug(`${this.label()} address lookup failed: ${describeError(error)}`)
      return false
    }
  }
  ...
  private label(): string {
    return `${forLog(this.options.displayName)} [${forLog(this.options.deviceId)}]`
  }
  ...
  private async sleepInterruptibly(ms: number): Promise<void> {
    const pending = interruptibleSleep(ms)
