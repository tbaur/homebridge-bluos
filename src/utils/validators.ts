...
import { isValidPlayerId } from '../api/identity'
import {
  DEFAULT_BLUOS_PORT,
...
import {
  isSliderService,
  type BluOSDeviceConfig,
  type BluOSPlatformConfig,
 ...
 */
export function isNonPrivateIpv4(value: string): boolean {
  if (!isIpv4(value)) {
    return false
  }
 ...
 */
const PROBE_LOCAL_SUFFIXES = ['.local', '.localhost', '.internal', '.home.arpa'] as const
 ...
 */
export function isProbeableHost(value: unknown): value is string {
  ...
  return PROBE_LOCAL_SUFFIXES.some(
 ...
 */
export function isDiscoveryHost(value: unknown): value is string {
...
export function isLoopbackIpv4(value: string): boolean {
  if (!isIpv4(value)) {
    return false
  }
...
export function resolveDiscoveryTimeoutSec(value: unknown, warnings?: string[]): number {
  if (value === undefined) {
    return DEFAULT_DISCOVERY_TIMEOUT_SEC
  }
  const numeric = typeof value === 'number' ? value : Number(value)
  if (!Number.isFinite(numeric)) {
    warnings?.push(
      `options.discoveryTimeoutSec is not a number; using ${DEFAULT_DISCOVERY_TIMEOUT_SEC}s`,
    )
    return DEFAULT_DISCOVERY_TIMEOUT_SEC
  }
  const clamped = Math.min(MAX_DISCOVERY_TIMEOUT_SEC, Math.max(MIN_DISCOVERY_TIMEOUT_SEC, Math.round(numeric)))
  if (clamped !== numeric) {
 ...
 */
export function resolveSliderService(
  deviceValue: unknown,
  ...
  for (const candidate of [deviceValue, platformValue]) {
    ...
    if (isSliderService(candidate)) {
      return candidate
    }
    warnings?.push(
      `${where}sliderService ${JSON.stringify(forLog(candidate))} is not "fan" or "lightbulb"; `
      + 'using "fan"',
    )
    return 'fan'
  }
...
function validatePort(value: unknown, label: string, warnings: string[]): number {
  if (value === undefined) {
    return DEFAULT_BLUOS_PORT
  }
  const numeric = typeof value === 'number' ? value : Number(value)
  if (!Number.isInteger(numeric) || numeric < MIN_PORT || numeric > MAX_PORT) {
    warnings.push(`${label} port ${forLog(value)} is invalid; using ${DEFAULT_BLUOS_PORT}`)
    return DEFAULT_BLUOS_PORT
  }
  if (!DOCUMENTED_BLUOS_PORTS.includes(numeric)) {
    // Accepted anyway: the SRV record, not this list, is the authority on which
    // port a zone listens to.
    warnings.push(
      `${label} uses port ${numeric}, which is outside the documented BluOS ports `
      + `(${DOCUMENTED_BLUOS_PORTS.join(', ')})`,
    )
  }
  return numeric
}
...
function validatePresets(
  ...
  const presets: VolumePresetConfig[] = []
  ...
  value.forEach((entry, index) => {
    ...
    const candidate = entry as { name?: unknown; volume?: unknown }
    ...
    if (!Number.isInteger(volume) || volume < VOLUME_MIN || volume > VOLUME_MAX) {
      problems.push(`${presetLabel} volume must be an integer ${VOLUME_MIN}-${VOLUME_MAX}`)
    }
...
function validateDevice(input: {
  ...
  const device = entry as Partial<BluOSDeviceConfig>
  const problems: string[] = []

  const name = validateName(device.name, label, problems)
  if (!isValidPlayerId(device.id)) {
