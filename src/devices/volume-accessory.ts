...
import { restoreLevelFrom } from '../api/sync-status'
import {
...
interface SliderSurface {
  ...
  /** Fanv2 `Active` is 1/0 while Lightbulb `On` is a boolean. */
  toActiveValue(on: boolean): CharacteristicValue
}

/** A volume slider for one player. */
export class VolumeAccessory extends BaseAccessory {
  private readonly surface: SliderSurface
  ...
  private readActive(): CharacteristicValue {
    this.requireSlider()
    return this.surface.toActiveValue((this.level ?? 0) > VOLUME_MIN)
  }

  private readLevel(): CharacteristicValue {
    this.requireSlider()
    return this.level ?? VOLUME_MIN
  }
  ...
  private async writeActive(value: CharacteristicValue): Promise<void> {
    const { Characteristic: Char } = this.host.hap
    const on = this.context.sliderService === 'lightbulb'
      ? value === true
      : value === Char.Active.ACTIVE
    if (!on) {
      await this.queueLevel(VOLUME_MIN)
      return
    }
    const observation = this.host.observationFor(this.deviceId)
    const restore = restoreLevelFrom(
      observation ?? {},
      this.context.lastNonZeroVolume,
      DEFAULT_RESTORE_VOLUME,
    )
    await this.queueLevel(restore)
  }

  private async writeLevel(value: CharacteristicValue): Promise<void> {
    const requested = typeof value === 'number' ? Math.round(value) : Number.NaN
    if (!Number.isInteger(requested)) {
      this.host.log.debug(`${forLog(this.displayName)} ignoring non-numeric level ${String(value)}`)
      return
    }
    await this.queueLevel(Math.min(VOLUME_MAX, Math.max(VOLUME_MIN, requested)))
  }
   ...
   */
  private async queueLevel(level: number): Promise<void> {
    this.pendingLevel = level
    if (this.flush === undefined) {
      this.flush = sleep(SLIDER_COALESCE_MS).then(async () => this.applyPendingLevel())
    }
    const flush = this.flush
    await this.completeWithinBudget('volume write', async () => flush)
  }

  private async applyPendingLevel(): Promise<void> {
    ...
    if (this.fixedVolume) {
      this.warnOnce(
        'fixed-volume-write',
        `${forLog(this.displayName)} reports a fixed output level; ignoring the volume write`,
      )
      return
    }
    ...
    if (level > VOLUME_MIN && observation?.muted === true) {
    ...
    if (result.level !== undefined && result.level !== level) {
      this.warnOnce(
        'clamped',
        `${forLog(this.displayName)} clamped volume ${level} to ${result.level}; `
  ...
  protected override updateFromObservation(
    ...
    if (observation.fixedVolume) {
      if (!this.fixedVolume) {
        ...
        this.warnOnce(
          'fixed-volume',
          `${forLog(this.displayName)} reports a fixed output level, so its volume slider `
    ...
    const level = observation.muted ? VOLUME_MIN : observation.volume ?? VOLUME_MIN
    this.level = level
    if (level > VOLUME_MIN && this.context.lastNonZeroVolume !== level) {
    ...
    const { Characteristic: Char } = this.host.hap
    this.surface.service.updateCharacteristic(
      this.context.sliderService === 'lightbulb' ? Char.On : Char.Active,
      this.surface.toActiveValue(level > VOLUME_MIN),
    )
    this.surface.service.updateCharacteristic(
